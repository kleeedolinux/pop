...
| Statically bound native FFI | syntax, type checker, HIR/MIR, PLRI/runtime, manifests/artifacts, linker, generator, LLVM | exact ABI/layout/effect identities, canonical hashed link plans, read-only pin and buffer-borrow regions, handle/callback/root proofs, native fixtures, and no raw flags/shell/runtime lookup/object-address tests |
